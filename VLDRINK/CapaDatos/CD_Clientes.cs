    ...
    public class CD_Clientes
    {
        public Cliente ObtenerClientePorCorreo(string correo)
        {
            ...
            try
            {
                ...
                {
                    string consulta = "SELECT IdCliente, Nombres, Apellidos, Correo, Reestablecer, FechaRegistro, PasswordHash, PasswordSalt FROM CLIENTE WHERE Correo = @correo";
        ...
        public int Registrar(Cliente obj, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection objConexion = new SqlConnection(Conexion.conex))
                {
                    string consulta = "INSERT INTO CLIENTE(Nombres, Apellidos, Correo, PasswordHash, PasswordSalt, Reestablecer, FechaRegistro) " +
                                      "VALUES(@Nombres, @Apellidos, @Correo, @PasswordHash, @PasswordSalt, @Reestablecer, GETDATE());" +
